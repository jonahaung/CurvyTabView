...
public struct CurvyTabView<SelectionValue, Background, Content> : View where SelectionValue : Hashable, Background : View, Content : View {
    ...
    @StateObject private var stateViewModel: CurvyTabStateViewModel<SelectionValue> = .init()
    private let viewModel: CurvyTabViewModel<SelectionValue>
    ...
    private let constant: CurvyTabViewConstants

    /// The style of the background view.
    public var background: ((CurvyTabState) -> Background)
    ...
    public var body: some View {
        GeometryReader { proxy in
            if proxy.size != .zero {
                ...
                .overlayPreferenceValue(CurvyTabItemPreferenceKey.self) { items in
                    if !items.isEmpty {
                        let items = items.prefix(getLimitItemCount(size: proxy.size, itemCount: items.count))
                        let state = CurvyTabState(constant: constant, itemCount: items.count, previousIndex: stateViewModel.previousIndex, currentIndex: stateViewModel.indexOfTag(selection.wrappedValue), size: proxy.size, safeAreaInsets: proxy.safeAreaInsets)
                        VStack(spacing: 0) {
    ...
    private func getTabContent(_ items: [CurvyTabItem]) -> some View {
...
public extension CurvyTabView where SelectionValue: Hashable, Background: View, Content: View {
    init(selection: Binding<SelectionValue>, constant: CurvyTabViewConstants = .init(), @ViewBuilder background: @escaping (CurvyTabState) -> Background, @ViewBuilder content: @escaping () -> Content, onTapReceive: ((SelectionValue) -> Void)? = nil) {
        self.viewModel = CurvyTabViewModel(selection: selection, constant: constant)
